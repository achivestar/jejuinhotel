                 ...
                 <div class="justify-content-end d-flex">
                	 <nav aria-label="breadcrumb">
					  <ol class="breadcrumb">
					    ...
					    <li class="breadcrumb-item active" aria-current="page">food서비스</li>
					  </ol>
					</nav>
                 </div>
                </div>
          ...
          <div class="row">
			...
			<div class="row" id="anch">
				<div class="col-md-12 text-right mb-3">
					<a href="/admin/foodWrite" class="btn btn-primary">food 소개 등록</a>
				</div>
				<div class="col-md-12">
					<div class="table-responsive-sm">
					  <table class="table">
							...
							<c:if test="${empty foodList}">
						        <tr><td colspan="4" class="text-center">등록된 소개가 없습니다.</td></tr>
							</c:if>
						    <c:forEach items="${foodList}" var="foodList" varStatus="status">	
							    <tr>
							      <th scope="row">${foodList.idx}</th>
							      <td><a href="/admin/foodModify?idx=${foodList.idx}">${foodList.title }</a></td>
							      <td>${foodList.gubun }</td>
							      <td>${foodList.regdate }</td>
							    </tr>
							</c:forEach>	   
							  </tbody>
					  </table>
