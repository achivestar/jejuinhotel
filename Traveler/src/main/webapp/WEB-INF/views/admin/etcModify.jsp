				...
				<div class="col-md-12">
					<form name="dataForm" id="dataForm"> <!-- onsubmit="return registerAction()" -->
						<div class="form-group">
						    ...
						    <input type="text" class="form-control" id="title" name="title" value="${etcvo.title }">
						</div>
						<div class="form-group">
							 <label for="content">부대시설소개 (간단히)</label>
							  <input type="text" class="form-control"  name="content" id="content" value="${etcvo.content }">
						</div>
						...
						<div class="form-group">
						    ...
						    <span style="font-size:10px; color: gray;">※첨부파일은 3개만 등록해야 합니다.</span>
						  	<div class="data_file_txt" id="data_file_txt">
								<span>첨부 파일</span>
								<br />
								<div id="articlefileChange"></div>
								<p>현재 적용된 이미지</p>
								<input type="hidden" name="idx" value="${etcvo.idx}" />
								<input type="hidden" name="count" value="${etcvo.count}" />
								<c:if test="${not empty etcvo.savedFileName1 }">
									<img src="/resources/etcfileupload/${etcvo.idx}/${etcvo.savedFileName1}" alt="" class="img-thumbnail" style="width:200px;height:auto">
									<input type="hidden" name="originalFileName1" value="${etcvo.originalFileName1}" />
									<input type="hidden" name="savedFileName1" value="${etcvo.savedFileName1}" />
								</c:if>
								<c:if test="${not empty etcvo.savedFileName2 }">
									<img src="/resources/etcfileupload/${etcvo.idx}/${etcvo.savedFileName2}" alt="" class="img-thumbnail" style="width:200px;height:auto">
									<input type="hidden" name="originalFileName2" value="${etcvo.originalFileName2}" />
									<input type="hidden" name="savedFileName2" value="${etcvo.savedFileName2}" />
								</c:if>
								<c:if test="${not empty etcvo.savedFileName3 }">
									<img src="/resources/etcfileupload/${etcvo.idx}/${etcvo.savedFileName3}" alt="" class="img-thumbnail" style="width:200px;height:auto">
									<input type="hidden" name="originalFileName3" value="${etcvo.originalFileName3}" />
									<input type="hidden" name="savedFileName3" value="${etcvo.savedFileName3}" />
								</c:if>
								<%-- <c:if test="${not empty etcvo.savedFileName4 }">
									<img src="/resources/etcfileupload/${etcvo.idx}/${etcvo.savedFileName4}" alt="" class="img-thumbnail" style="width:200px;height:auto">
									<input type="hidden" name="originalFileName4" value="${etcvo.originalFileName4}" />
									<input type="hidden" name="savedFileName4" value="${etcvo.savedFileName4}" />
								</c:if>
								<c:if test="${not empty etcvo.savedFileName5 }">
									<img src="/resources/etcfileupload/${etcvo.idx}/${etcvo.savedFileName5}" alt="" class="img-thumbnail" style="width:200px;height:auto">
									<input type="hidden" name="originalFileName5" value="${etcvo.originalFileName5}" />
									<input type="hidden" name="savedFileName5" value="${etcvo.savedFileName5}" />
								</c:if> --%>
